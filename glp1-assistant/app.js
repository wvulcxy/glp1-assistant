...
// 漏针后仍可补打的最长时间。每日制剂没有“补打一针”的规则，
// 因此只保留当前给药周期，下一次计划时间到达后自动跳过。
...
function getNextInjection(data, now = new Date()) {
  ...
  // 本针打完后才进入下一周期；在说明书允许的补打窗口内始终保持“待注射”。
...
function updateNotifyStatus() {
  ...
  if (Notification.permission === 'granted') {
    ...
    el.innerHTML = '🚫 通知已被拒绝 · 请在浏览器设置中开启';
  } else {
...
function checkAndNotify() {
  ...
  const now = new Date();
  const diff = next - now;
  // Notify if within 30 minutes of injection time
  if (diff > 0 && diff <= 30 * 60 * 1000) {
    ...
    new Notification('💉 GLP-1 用药提醒', {
      body: `该注射 ${drugName} 了！剂量：${data.reminder.dose}mg`,
...
console.log('🔔 GLP-1 用药助手 v1.3 已就绪');
